...
for image_name in image_dir: 
  ...
  path_ENV = '/content/colaba/MyDrive/colab/image-compression-master/'
  # save the plot and the map
  if not os.path.exists('output'):
      os.makedirs('output')
  image_map_file = path_ENV + 'images_directory/output_kodak/map_' + image_name_path.split('/')[-1][:-4] + '.jpg'
  plt.savefig('output/overlayed_heatmap.png')
  skimage.io.imsave( image_map_file, roi_map )
  time.sleep(1)
