...
if hyper.sparse:
    loss_tf   = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prob_tf, labels=labels_tf))
else:
    loss_tf   = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prob_tf, labels=labels_tf))
train_loss    = tf.summary.scalar("training loss", loss_tf)
test_loss     = tf.summary.scalar("validation loss", loss_tf)
...
with tf.Session() as sess:
    ...
    for epoch in range(tparam.num_epochs):
        ...
        for b, train_batch in enumerate(chunker(data_train.sample(frac=1),tparam.batch_size)):
            train_images  = np.array(list(map(lambda i: load_image(i), train_batch['image_path'].values)))
            train_labels  = sparse_labels_or_not(train_batch)
            _, batch_loss, loss_sw = sess.run([train_op, loss_tf, train_loss], feed_dict={images_tf: train_images, labels_tf: train_labels, cnn.is_train: True})
        ...
        for b, test_batch in enumerate(chunker(data_test,tparam.batch_size)): # no need to randomize test batch
            test_images        = np.array(list(map(lambda i: load_image(i), test_batch['image_path'].values)))
            # don't run the train_op by mistake ! ;-) 
            test_labels        = sparse_labels_or_not(test_batch)
            batch_loss,loss_sw = sess.run([loss_tf, test_loss], feed_dict={images_tf: test_images, labels_tf: test_labels, cnn.is_train: False})
        ...
        # save the model
        if min_epoch_loss > (validation_loss/b):
          saver.save(sess, tparam.model_path + '/model')
          min_epoch_loss = validation_loss/b
