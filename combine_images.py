...
def make_quality_compression(original, sal):
    ...
    #os.system('sips -s format png ' + args.image + " --out "+ args.output_directory + '/temp.png')
...
if not os.path.exists(args.output_directory):  # 生成output文件夹
    os.makedirs(args.output_directory)

if args.print_metrics:
    from get_metrics import get_metrics

if args.single:  # default为1
    original = Image.open(args.image)  # 打开原图
    sal = Image.open(args.map)  # 打开map
    ...
    for image_file in glob(image_path):
        if args.dataset == 'large':
            ...
            map_file = 'images_directory/output_kodak/map_' + image_file.split('/')[-1][:-4] + '.jpg'
